# ASAS-21
# python3 /users/darinadaly/lmdeb//py_files/ASAS09.py
...
# DATA ACCESS
lit_period = 0.7024303
lit_t0 = 2450000 + 5009.86436 - 2457000  # t0 = 0 at peak of primary eclipse

# Prior on the flux ratio for Kepler
lit_flux_ratio = (0.44, 0.2)

import numpy as np
import matplotlib.pyplot as plt
import lightkurve as lk

tpf = lk.search_targetpixelfile("ASAS J093814-0104.04", mission=('TESS')).download()
...
rvs = np.array(
    [
        (3903.6543 + ref_date, -125.31, 112.00),
        (3903.6949 + ref_date, -141.05, 130.09),
        (3904.6314 + ref_date, 32.74, -72.70),
        (3905.5318 + ref_date, 87.00, -144.90),
        (3906.5668 + ref_date, -135.15, 123.63),
        (3906.6109 + ref_date, -105.44, 85.47),
        (3906.6489 + ref_date, -67.42, 51.39),
        (4727.0620 + ref_date, -97.72, 76.92),
        (4728.0651 + ref_date, 95.93, -148.10),
...
def build_model(mask):
    with pm.Model() as model:
        ...
        M1 = pm.Lognormal("M1", mu=0.0, sigma=10.0, testval=0.833)
        R1 = pm.Lognormal("R1", mu=0.0, sigma=10.0, testval=0.845)

        # Secondary ratios
        k = pm.Lognormal("k", mu=0.0, sigma=10.0)  # radius ratio
        q = pm.Lognormal("q", mu=0.0, sigma=10.0, testval=0.85)  # mass ratio
        s = pm.Lognormal("s", mu=np.log(.5), sigma=10.0)  # surface brightness ratio
...
plt.ylabel("radial velocity [km / s]")
plt.xlabel("time since primary eclipse [days]")
_ = plt.title("ASAS-21; map model", fontsize=14)
plt.show()
...
ax1.set_title("ASAS-21; map model", fontsize=14)
plt.show()
...
_ = ax1.set_title("ASAS-21; map model", fontsize=14)
plt.show()
